...
def test_phonebook_delete_requires_slug_confirmation(client):
    client.post(
        "/login",
        data={"username": "admin", "password": "admin123"},
        follow_redirects=True,
    )
    client.post("/phonebooks", data={"name": "Delete Me"}, follow_redirects=True)
    ...
    assert b"Slug confirmation does not match." in denied.data

    ok = client.post(
        "/phonebooks/1/delete",
        data={"confirm_slug": "delete-me"},
        follow_redirects=True,
    )
